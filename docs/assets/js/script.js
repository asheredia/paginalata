...
AWS.config.credentials = new AWS.CognitoIdentityCredentials({
    IdentityPoolId: 'us-east-1:2ba90b79-486f-4310-b6b1-30d883d5e33f',
});
    ...
    function onScan(err, data) {
        if (err) {
            ...
            stations.forEach(function (id) {
                ...
                var params = {
                    TableName: "DISPENSADOR_IOT",
                    ProjectionExpression: "id, #dtime, distancia, temperatura, tempc",
                ...
                docClient.query(params, function (err, data) {
                    if (err) {
                        ...
                        document.getElementById('latest-value1').innerHTML = latest.distancia + ' m';
                        document.getElementById('latest-value2').innerHTML = latest.temperatura + '°C';
                        document.getElementById('latest-value3').innerHTML = latest.tempc + '°C';
                       ...
                       // document.getElementById('latest-value5').innerHTML = latest.rainHeight + ' mm/h';
                        ...
                        for (c = 0; c < 5; c++) {
                            document.getElementById('station' + id).innerHTML += '<tr><td>' + actual[c].id + '</td><td>' + actual[c].datetime + '</td><td>' + actual[c].distancia + '</td><td>' + actual[c].temperatura +
                                '</td><td>' + actual[c].tempc +'</td></tr> ';
                        }
                    }
    ...
    function onScan(err, data) {
        if (err) {
            ...
            for (c = 0; c < sizeList; c++) {
                if (sensor === "distancia")
                    document.getElementById('id' + sensor).innerHTML += '<tr><td>' + items[c].datetime + '</td><td>' + items[c].id + '</td><td class="text-primary">' + items[c].distancia + '</td></tr> ';
                ...
                else if (sensor === "tempc")
                    document.getElementById('id' + sensor).innerHTML += '<tr><td>' + items[c].datetime + '</td><td>' + items[c].id + '</td><td class="text-primary">' + items[c].tempc + '</td></tr> ';
